...
namespace wt
{
    template <const uint BM, const uint BN, const uint BK,
              const uint strideA, const uint strideB>
    __device__ void loadFromGMEM(float *chunkA, float *chunkB,
                                 float *AS, float *BS,
                                 const uint K, const uint N,
                                 const uint innerRowA, const uint innerColA,
                                 const uint innerRowB, const uint innerColB)
    {
        ...
        {
            const float4 tmp = reinterpret_cast<float4 *>(&chunkA[(innerRowA + offset) * K + innerColA * 4])[0];
        ...
        {
            reinterpret_cast<float4 *>(&BS[(innerRowB + offset) * BN + innerColB * 4])[0] = reinterpret_cast<float4 *>(&chunkB[(innerRowB + offset) * N + innerColB * 4])[0];
        }
    }

    template <const uint BM, const uint BN, const uint BK,
              const uint WMITER, const uint WM, const uint WSUBM, const uint WNITER, const uint WN, const uint WSUBN,
              const uint TM, const uint TN>
    __device__ void processFromSMEM(float *regM, float *regN, float *threadResults, float *AS, float *BS,
...
template <const uint BM, const uint BN, const uint BK,
          const uint WM, const uint WN, const uint WNITER,
          const uint TM, const uint TN, const uint N_THREADS>
__global__ void __launch_bounds__(N_THREADS)
    warpTile(float *__restrict__ A, float *__restrict__ B, float *C, const uint M, const uint N, const uint K)
{
    ...
    for (uint chunk{0}; chunk < K; chunk += BK)
    {
        ...
        wt::loadFromGMEM<BM, BN, BK, strideA, strideB>(A, B, AS, BS, K, N, innerRowA, innerColA, innerRowB, innerColB);
        __syncthreads();
        // move from shared mem to reg then do outer product
        wt::processFromSMEM<BM, BN, BK, WMITER, WM, WSUBM, WNITER, WN, WSUBN, TM, TN>(regM, regN, threadResults, AS, BS, warpRow, warpCol, threadRowInWarp, threadColInWarp);
                ...
                for (uint resIdxN = 0; resIdxN < TN; resIdxN += 4)
                {
                    reinterpret_cast<float4 *>(
                        &C_interim[(threadRowInWarp * TM + resIdxM) * N +
                                   threadColInWarp * TN + resIdxN])[0] = reinterpret_cast<float4 *>(&threadResults[(wSubRowIdx * TM + resIdxM) * (WNITER * TN) +
...
int main()
{
    ...
    // launch the kernel
    warpTile<BM, BN, BK, WM, WN, WNITER, TM, TN, N_THREADS><<<grid, block>>>(d_A, d_B, d_C, M, N, K);
